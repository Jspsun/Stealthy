...
public class MapMethods extends JPanel implements ActionListener
{
	...
	public void newGame()
	{
		...
		// Sets up the map grid at the very beginnng and repaints the display
	...
	public MapMethods()
	{
		...
		// Sound file for the background music
		backgroundMusic = Applet
				.newAudioClip(getCompleteURL("Music.wav"));
	...
	public void setUpMapGrid(String mapFile, String itemMapFile)
	{
		...
		if (!mapIsExplored[currentMapRow][currentMapColumn])
		{
			try
			{
				...
				// Runs through the newly created mapGrid array, and for every
				// occurrence of the number 7, a robot is created at that
				// location and the number is reassigned to 0
				int index = 0;
				for (int row = 1; row < mapGrid.length - 1; row++)
				{
					for (int column = 1; column < mapGrid[0].length - 1; column++)
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		musicTime++;
		if (musicIsOn && musicTime % 57000 == 0)
		{
			backgroundMusic.play();
		}
